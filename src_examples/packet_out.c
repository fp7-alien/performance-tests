...
//local mac
char local_mac[] = {0x08, 0x00, 0x27, 0xfb, 0x45, 0x95};
  ...
  for (np = head.tqh_first; np != NULL; np = np->entries.tqe_next) {
    ...
    if(print) {
      snprintf(msg, 1024, "%lu.%06lu:%lu.%06lu:%d",
	       ...
	       np->id); 
      oflops_log(now, OFPT_PACKET_OUT_MSG, msg);
    }
    free(np);
  }
...
generate_pkt_out(struct oflops_context * ctx,struct timeval *now) {
  ...
  if(b == NULL) {
    ...
    memcpy(ether->ether_shost, "\x08\x00\x27\xbe\x63\xfe", ETH_ALEN);
//08:00:27:be:63:fe
    ...
    ip->saddr = inet_addr("30.30.30.2"); 
    ip->daddr = inet_addr("30.30.30.3"); //test.nw_dst;
  ...
  ip->daddr = htonl(ntohl(inet_addr("30.30.30.4")) + (rand()%(flows)) ); //test.nw_dst;
