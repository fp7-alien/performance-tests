...
//08:00:27:be:63:fe 
  ...
  fl->nw_src =  inet_addr("10.1.1.2");
  fl->nw_dst =  inet_addr("10.1.1.1");
...
//    ch = np->ch - 1;
	...
	printf("Packet data transmition time:  %d, <--> %f : ch: %d\n",ix-1, data[ix-1],  np->ch);
    free(np);
  }
    ...
    if(ix > 0) {
      ...
      printf("statistics:port:%d:%u:%u:%u:%.4f:%d\n", 
	     ctx->channels[ch_n].of_port, mean, median, std, loss, count);
      oflops_log(now, GENERIC_MSG, msg);
    }
  ...
  for(i = 1; i <= echo_data_count; i++) {
    ...
    oflops_log(now, GENERIC_MSG, msg);
    data[i-1] = time_diff(& echo_data[i].snd, &echo_data[i].rcv);
  }
  if(echo_data_count > 0) {
    gsl_sort (data, 1, echo_data_count);
    mean = (uint32_t)gsl_stats_mean(data, 1, echo_data_count);
    std = (uint32_t)sqrt(gsl_stats_variance(data, 1, echo_data_count));
    median = (uint32_t)gsl_stats_median_from_sorted_data (data, 1, echo_data_count);
...
//08:00:27:eb:82:81 
    memcpy(fl_probe->dl_dst, "\x08\x00\x27\xeb\x82\x81", 6);
